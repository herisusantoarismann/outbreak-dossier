...
describe("Exhaustive Bilingual Purity & Integrity Across All Pandemics", () => {
    ...
    it("enforces strict bilingual title, description, date, and flash parity across every chapter", () => {
        for (const sector of sectorFiles) {
            sector.chapters.forEach((chapter, index) => {
                ...
                expect(
                    chapter.title,
                    `${context} title must be defined`,
                ).toBeDefined();
                expect(
                    chapter.title.id?.trim(),
                    `${context} title.id must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.title.en?.trim(),
                    `${context} title.en must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.title.id.trim(),
                ...
                expect(
                    chapter.description,
                    `${context} description must be defined`,
                ).toBeDefined();
                expect(
                    chapter.description.id?.trim(),
                    `${context} description.id must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.description.en?.trim(),
                    `${context} description.en must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.description.id.trim(),
                ...
                expect(
                    chapter.date,
                    `${context} date must be defined`,
                ).toBeDefined();
                expect(
                    chapter.date.id?.trim(),
                    `${context} date.id must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.date.en?.trim(),
                    `${context} date.en must not be empty`,
                ).toBeTruthy();

                // Flash check
                expect(
                    chapter.flash,
                    `${context} flash must be defined`,
                ).toBeDefined();
                expect(
                    chapter.flash.id?.trim(),
                    `${context} flash.id must not be empty`,
                ).toBeTruthy();
                expect(
                    chapter.flash.en?.trim(),
                    `${context} flash.en must not be empty`,
                ).toBeTruthy();
            });
        }
    });
        ...
        for (const sector of sectorFiles) {
            sector.chapters.forEach((chapter, index) => {
                ...
                expect(
                    ...
                    `${context} virusProfile must be defined`,
                ).toBeDefined();
                expect(
                    chapter.virusProfile.agent,
                    `${context} virusProfile.agent must be defined`,
                ).toBeTruthy();
                expect(
                    chapter.virusProfile.vector,
                    `${context} virusProfile.vector must be defined`,
                ).toBeTruthy();
                expect(
                    chapter.virusProfile.transmission,
                    `${context} virusProfile.transmission must be defined`,
                ).toBeTruthy();

                if (chapter.virusProfile.mutationType) {
                    expect(
                        chapter.virusProfile.mutationType.id?.trim(),
                        `${context} mutationType.id`,
                    ).toBeTruthy();
                    expect(
                        chapter.virusProfile.mutationType.en?.trim(),
                        `${context} mutationType.en`,
                    ).toBeTruthy();
                }

                if (chapter.virusProfile.threatLevel) {
                    expect(
                        chapter.virusProfile.threatLevel.id?.trim(),
                        `${context} threatLevel.id`,
                    ).toBeTruthy();
                    expect(
                        chapter.virusProfile.threatLevel.en?.trim(),
                        `${context} threatLevel.en`,
                    ).toBeTruthy();
                }

                if (chapter.virusProfile.clinicalTarget) {
                    expect(
                        chapter.virusProfile.clinicalTarget.id?.trim(),
                        `${context} clinicalTarget.id`,
                    ).toBeTruthy();
                    expect(
                        chapter.virusProfile.clinicalTarget.en?.trim(),
                        `${context} clinicalTarget.en`,
                    ).toBeTruthy();
                }
            });
        }
    ...
    it("validates UI messages localization parity between src/messages/en.json and id.json", () => {
        ...
        expect(
            missingInId,
            "Keys present in en.json but missing in id.json",
        ).toEqual([]);
        expect(
            missingInEn,
            "Keys present in id.json but missing in en.json",
        ).toEqual([]);

        // Verify meta description is properly translated to Indonesian
        expect(idMessages.meta.title).toContain("Arsip Patogen Global");
        expect(idMessages.meta.description).toContain(
            "Kronik intelijen visual deklasifikasi",
        );
    });
});
